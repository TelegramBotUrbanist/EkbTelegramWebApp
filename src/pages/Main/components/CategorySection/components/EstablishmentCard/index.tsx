...
const EstablishmentCard: React.FC<EstablishmentCardProps> = ({
  ...
  const handleNavigate = () => {
    return  type === 'establishments' ? navigate(`/establishment/${establishment.id}`) : type == 'events' ?  navigate(`/events/${0}`) : setIsDetailsOpen(true)
  }
...
const ModalSheetContainer = ({establishment,isSheetOpen,setIsSheetOpen,selectedReservationIds,handleReservationSelect,onReservationSelect,navigate}) => {
  ...
  const handleNavigate = () => {
    return  establishment.entityType === 'FOOD_ESTABLISHMENT' ? navigate(`/establishment/${establishment.id}`) :  navigate(`/events/${0}`)
  }
  return (
      <Sheet ref={ref}
