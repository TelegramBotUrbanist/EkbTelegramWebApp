    ...
    return (
      <motion.div className="hours-component">
        <div className="hours-header" onClick={() => setShowHours(!showHours)}>
          <span>
            {currentDay ? !currentDay?.open ? 'Выходной' :  `Открыто с ${currentDay ? formatTime(currentDay.from) : ''} до ${currentDay ? formatTime(currentDay.till) : ''}`: "График работы"}
          </span>
          <img
            src={`/dropdown.svg`}
          ...
          <ul className="hours-list">
            {openingHours.map((hours, index) => (
              <li key={index} className={`hours-item ${hours.currentDay ? 'current-day' : ''}`}>
                ...
                <span className="time">
                  {!hours.open ? 'Выходной' : `${formatTime(hours.from)} – ${formatTime(hours.till)}`}
                </span>
              </li>
            ))}
          </ul>
